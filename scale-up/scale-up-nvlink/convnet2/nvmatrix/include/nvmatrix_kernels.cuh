...
template<typename T> 
__device__ T __shfl_down_sync(T a, int b, int c=WARP_SIZE) {
...
__global__ void kAggRows_wholerow_nosync(const float* mat, float* matSum, const uint width, const uint height,
    ...
    for (uint i = 0; i < LOG_WARP_SIZE; i++) {
        const uint d = 1 << i;
        myAccum[0] = agg(myAccum[0], __shfl_down_sync(myAccum[0], d));
    }
    __syncthreads();
    ...
    if (tidx < AWR_NUM_WARPS) {
        ...
        for (uint i = 0; i < AWR_LOG_NUM_WARPS; i++) {
            const uint d = 1 << i;
            myFinalAccum[0] = agg(myFinalAccum[0], __shfl_down_sync(myFinalAccum[0], d));
        }
        if (tidx == 0) {
